    ...
    Surface(
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            ...
            question?.let {
                ...
                Text(
                    text = it.optionA,
                    modifier = Modifier.clickable { checkAnswer(it.optionA, it.answer) })
                Text(
                    text = it.optionB,
                    modifier = Modifier.clickable { checkAnswer(it.optionB, it.answer) })
                Text(
                    text = it.optionC,
                    modifier = Modifier.clickable { checkAnswer(it.optionC, it.answer) })
                Text(
                    text = it.optionD,
                    modifier = Modifier.clickable { checkAnswer(it.optionD, it.answer) })
            }
        }
    }
}
